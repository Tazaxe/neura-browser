...
from PyQt6.QtWebEngineCore import QWebEngineProfile, QWebEnginePage, QWebEngineDownloadRequest, QWebEngineSettings, QWebEngineUrlRequestInterceptor
...
# ─────────────────────────────────────────────
VERSION        = "4.0.2"          # versión actual del archivo
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        sep1.setStyleSheet(f"color:{OUTLINE};")
        lay.addWidget(sep1)

        ab_lbl = QLabel("AD_BLOCKER")
        ab_lbl.setFont(mono(8, True))
        ab_lbl.setStyleSheet(f"color:{TEXT_DIM};letter-spacing:2px;")
        lay.addWidget(ab_lbl)

        self.adblock_chk = QCheckBox("Bloquear anuncios y rastreadores")
        self.adblock_chk.setFont(mono(9))
        self.adblock_chk.setStyleSheet(f"QCheckBox{{color:{PRIMARY};}}")
        lay.addWidget(self.adblock_chk)

        sep2 = QFrame()
        sep2.setFrameShape(QFrame.Shape.HLine)
        sep2.setStyleSheet(f"color:{OUTLINE};")
        lay.addWidget(sep2)

        vpn_lbl = QLabel("VPN / PROXY  (servidor propio — requiere reiniciar)")
        vpn_lbl.setFont(mono(8, True))
        vpn_lbl.setStyleSheet(f"color:{TEXT_DIM};letter-spacing:1px;")
        lay.addWidget(vpn_lbl)

        self.vpn_chk = QCheckBox("Habilitar VPN/Proxy")
        self.vpn_chk.setFont(mono(9))
        self.vpn_chk.setStyleSheet(f"QCheckBox{{color:{PRIMARY};}}")
        lay.addWidget(self.vpn_chk)

        self.proxy_in = QLineEdit()
        self.proxy_in.setPlaceholderText("host:puerto  o  socks5://host:puerto")
        self.proxy_in.setFont(mono(9))
        self.proxy_in.setStyleSheet(f"QLineEdit{{background:{BG3};color:{PRIMARY};border:1px solid {OUTLINE};padding:6px 10px;}}QLineEdit:focus{{border-color:{PRIMARY};}}")
        lay.addWidget(self.proxy_in)

        s = load_settings()
        self.adblock_chk.setChecked(s.get("adblock_enabled", True))
        self.vpn_chk.setChecked(s.get("vpn_enabled", False))
        self.proxy_in.setText(s.get("proxy_server", ""))
    ...
    def _save(self):
        ...
        s = load_settings()
        prev_proxy = (s.get("proxy_server", ""), s.get("vpn_enabled", False))
        s["adblock_enabled"] = self.adblock_chk.isChecked()
        s["vpn_enabled"] = self.vpn_chk.isChecked()
        s["proxy_server"] = self.proxy_in.text().strip()
        save_settings(s)

        new_proxy = (s["proxy_server"], s["vpn_enabled"])
        if new_proxy != prev_proxy:
            self.status.setStyleSheet(f"color:{TERTIARY};")
            self.status.setText("// GUARDADO ✓  (proxy/VPN: reinicia Neura)")
...
class TabBtn(QWidget):
    ...
    def __init__(self, title="NEW_TAB", index=0, parent=None):
        ...
        self.close_btn.setAttribute(Qt.WidgetAttribute.WA_TransparentForMouseEvents, True)
    ...
    def mousePressEvent(self, e):
        if e.button() == Qt.MouseButton.LeftButton and self.close_btn.geometry().contains(e.pos()):
...
class TabBar(QWidget):
    ...
    tab_torn_off = pyqtSignal(int, object)  # from_idx, drop_global_pos
    ...
    def start_drag(self, from_idx):
        ...
        # Si el drop no fue aceptado por ningún TabBar (resultado IgnoreAction)
        # significa que lo soltaron fuera → abrir en nueva ventana
        if result == Qt.DropAction.IgnoreAction:
            pos = QCursor.pos()
            self.tab_torn_off.emit(from_idx, pos)
...
# ─────────────────────────────────────────────
#  Ad Blocker — bloquea peticiones de red a dominios
#  conocidos de publicidad y rastreo
# ─────────────────────────────────────────────
AD_BLOCK_DOMAINS = [
    ...
    "advertising.com", "adsystem.com", "amazon-adsystem.com",
    "rubiconproject.com", "pubmatic.com", "openx.net", "casalemedia.com",
    "yieldmo.com", "bidswitch.net", "media.net", "mgid.com",
    "popads.net", "propellerads.com", "exoclick.com", "adcolony.com",
    "applovin.com", "innovid.com", "doubleverify.com", "branch.io",
    "appsflyer.com", "adform.net", "smartadserver.com", "yandex.ru/ads",
]
...
class AdBlockInterceptor(QWebEngineUrlRequestInterceptor):
    ...
    def interceptRequest(self, info):
        ...
        if any(host == d or host.endswith("." + d) for d in AD_BLOCK_DOMAINS):
            info.block(True)
...
class NeuraPage(QWebEnginePage):
    ...
    def _on_fullscreen_requested(self, request):
        toggle_on = request.toggleOn()
        try:
            request.accept()
        except Exception:
            return
        # PyQt6: QWebEnginePage no tiene .view() — buscar el QWebEngineView en el árbol de parents
        from PyQt6.QtWebEngineWidgets import QWebEngineView
        view = None
        p = self.parent()
        while p is not None:
            if isinstance(p, QWebEngineView):
                view = p
                break
            p = p.parent()
        # Buscar la ventana principal con _set_webview_fullscreen
        win = view.window() if view else None
        if win is None or not hasattr(win, "_set_webview_fullscreen"):
            for widget in QApplication.topLevelWidgets():
                if hasattr(widget, "_set_webview_fullscreen"):
                    win = widget
                    break
        if win is None:
            return
        QTimer.singleShot(0, lambda: win._set_webview_fullscreen(toggle_on, view) if win else None)
...
class NeuraApp(QMainWindow):
    def __init__(self, incognito=False):
        ...
        self._ad_interceptor = AdBlockInterceptor(self)
        self._ad_interceptor.enabled = load_settings().get("adblock_enabled", True)
        self._profile.setUrlRequestInterceptor(self._ad_interceptor)
        self._incognito_profile.setUrlRequestInterceptor(self._ad_interceptor)

        # Refuerzo: habilitar fullscreen también a nivel de perfil (no solo
        # por página) por si alguna versión de QtWebEngine lo requiere ahí.
        for prof in {self._profile, self._incognito_profile}:
            try:
                ps = prof.settings()
    ...
    def _set_webview_fullscreen(self, on, view=None):
        if on and not self._web_fullscreen:
            self._web_fullscreen = True
            self._pre_fullscreen_state = self.windowState()
            self.header.hide()
            self.tab_bar_wrap.hide()
            self.url_bar_widget.hide()
            self.progress.hide()
            self.statusBar().hide()
            if hasattr(self, "ai_panel") and self.ai_panel.isVisible():
                self._ai_was_visible = True
                self.ai_panel.hide()
            else:
                self._ai_was_visible = False
            self.showFullScreen()
            if view is None:
                view = self._current_browser()
            if view:
                view.setFocus()
        elif not on and self._web_fullscreen:
            self._web_fullscreen = False
            self.header.show()
            self.tab_bar_wrap.show()
            self.url_bar_widget.show()
            self.statusBar().show()
            if getattr(self, "_ai_was_visible", False):
                self.ai_panel.show()
    ...
    def _build_ui(self):
        ...
        # ═══ TAB BAR ═══
        self.tab_bar_wrap = tab_bar_wrap = QWidget()
        ...
        self.tab_container.tab_torn_off.connect(self._tear_off_tab)
        ...
        # ═══ URL / SEARCH BAR ═══
        self.url_bar_widget = url_bar_widget = QWidget()
        ...
        for icon, tip, slot in [
            ...
            ("⛶",  "Fullscreen (F11)",      lambda: self._set_webview_fullscreen(not self._web_fullscreen)),
    ...
    def _setup_shortcuts(self):
        shortcuts = [
            ...
            ("F12",              self._devtools),
            ("F11",              lambda: self._set_webview_fullscreen(not self._web_fullscreen)),
            ("Ctrl+Shift+N",     self._open_incognito),
            ("Ctrl+Shift+A",     self._toggle_ai_panel),
            ("Escape",           lambda: self._set_webview_fullscreen(False) if self._web_fullscreen else (self._current_browser() and self._current_browser().stop())),
        ]
    ...
    def _add_tab(self, url=None):
        ...
        tab_btn.on_select = lambda tb=tab_btn, br=browser: self._switch_tab(self._tab_index(tb, br))
        tab_btn.on_close  = lambda tb=tab_btn, br=browser: self._close_tab(self._tab_index(tb, br))
        tab_btn.close_btn.clicked.connect(
            lambda tb=tab_btn, br=browser: self._close_tab(self._tab_index(tb, br))
        )

        self._switch_tab(idx)

    def _tab_index(self, tab_btn, browser):
        for i, (tb, br) in enumerate(self.tabs):
            if tb is tab_btn and br is browser:
                return i
        return -1
    ...
    def _close_tab(self, idx):
        if idx < 0 or idx >= len(self.tabs): return
        if len(self.tabs) <= 1:
            # Última pestaña: en vez de no hacer nada, la reemplazamos
            # por una pestaña nueva en blanco (comportamiento esperado
            # de "cerrar" cuando solo queda una).
            tb, br = self.tabs[idx]
            try:
                br.setHtml(self._home_html(), QUrl("about:blank"))
            except Exception:
                pass
            return
        tb, br = self.tabs[idx]
        # Stop audio and navigation before destroying the widget
        try:
            br.stop()
            br.page().setAudioMuted(True)
            br.setUrl(QUrl("about:blank"))
        except Exception:
            pass
    ...
    def _tear_off_tab(self, idx, drop_pos):
        """Abre la pestaña idx en una nueva ventana NeuraApp y la cierra aquí."""
        if idx < 0 or idx >= len(self.tabs):
            return
        _, browser = self.tabs[idx]
        ...
        # Crear nueva ventana
        new_win = NeuraApp()
        # Posicionar cerca de donde se soltó
        if drop_pos:
            new_win.move(drop_pos.x() - 100, drop_pos.y() - 20)
        new_win.show()
        # Navegar a la URL de la pestaña arrastrada
        if url:
            bv = new_win._current_browser()
            if bv:
                bv.setUrl(QUrl(url))
        self._incognito_windows.append(new_win)  # mantener referencia viva

        # Cerrar la pestaña original (solo si queda más de una)
        if len(self.tabs) > 1:
            self._close_tab(idx)
    ...
    def _devtools(self):
        b = self._current_browser()
        if not b: return
        dev = QWebEngineView(self)
        b.page().setDevToolsPage(dev.page())
        dev.resize(900, 600)
        dev.setWindowTitle("NEURA // DevTools")
        dev.setWindowFlags(Qt.WindowType.Window)
        dev.show()
        self._devtools_win = dev

    def _on_download(self, item: QWebEngineDownloadRequest):
        default_dir = item.downloadDirectory() or os.path.join(os.path.expanduser("~"), "Downloads")
        if not os.path.isdir(default_dir):
            default_dir = os.path.expanduser("~")
        suggested = os.path.join(default_dir, item.suggestedFileName())
        ...
        if path:
            ...
            item.accept()
            entry = {"name": os.path.basename(path), "path": path, "state": "downloading", "received": 0, "total": item.totalBytes()}
            self._downloads.insert(0, entry)
            self.statusBar().showMessage(f"  ⬇ DOWNLOADING: {entry['name']}", 4000)

            def on_progress(received, total, e=entry):
                e["received"] = received
                e["total"] = total

            def on_state_changed(state, e=entry, it=item):
                st = it.state()
                if st == QWebEngineDownloadRequest.DownloadState.DownloadCompleted:
                    e["state"] = "completed"
                    self.statusBar().showMessage(f"  ✓ DOWNLOAD COMPLETE: {e['name']}", 4000)
                elif st == QWebEngineDownloadRequest.DownloadState.DownloadCancelled:
                    e["state"] = "cancelled"
                    self.statusBar().showMessage(f"  ✕ DOWNLOAD CANCELLED: {e['name']}", 4000)
                elif st == QWebEngineDownloadRequest.DownloadState.DownloadInterrupted:
                    e["state"] = "failed"
                    self.statusBar().showMessage(f"  ✕ DOWNLOAD FAILED: {e['name']}", 5000)

            item.receivedBytesChanged.connect(lambda e=entry, it=item: on_progress(it.receivedBytes(), it.totalBytes(), e))
    ...
    def _open_downloads(self):
        ...
        if not self._downloads:
            rows = '<div class="empty">// SIN DESCARGAS</div>'
        else:
            for d in self._downloads:
                size = d.get("total") or 0
                recv = d.get("received") or 0
                pct = int(recv * 100 / size) if size > 0 else (100 if d["state"] == "completed" else 0)
                state_label = {"completed": "✓ COMPLETO", "downloading": f"⬇ {pct}%", "cancelled": "✕ CANCELADO", "failed": "✕ ERROR"}.get(d["state"], d["state"])
                rows += f'<div class="row"><span class="fname">{d["name"]}</span><span class="fstate">{state_label}</span><span class="fpath">{d["path"]}</span></div>'

        html = f"""<!DOCTYPE html><html><head><meta charset="utf-8"><style>
        *{{box-sizing:border-box;margin:0;padding:0;}}
        body{{background:#000;color:{PRIMARY};font-family:'Courier New',monospace;padding:30px;}}
        h1{{font-size:20px;letter-spacing:6px;border-bottom:1px solid {OUTLINE};padding-bottom:12px;margin-bottom:20px;}}
        .row{{display:flex;flex-direction:column;gap:2px;border:1px solid {OUTLINE};padding:10px 14px;margin-bottom:8px;}}
        .fname{{font-size:13px;color:{PRIMARY};}}
        .fstate{{font-size:11px;color:{TERTIARY};}}
        .fpath{{font-size:10px;color:{TEXT_DIM};word-break:break-all;}}
        .empty{{font-size:12px;color:{TEXT_DIM};}}
        </style></head><body>
        <h1>// DESCARGAS</h1>
        {rows}
        </body></html>"""
        b = self._current_browser()
        if b:
            b.setHtml(html, QUrl("about:blank"))
    ...
    def _tick_status(self):
        ...
        self.statusBar().showMessage(
            f"  ● NEURA [V4.0.2]    //    {datetime.now().strftime('%H:%M:%S')}    //    NEURA_OS 2026"
        )
...
if __name__ == "__main__":
    ...
    if _settings.get("vpn_enabled") and _settings.get("proxy_server"):
        proxy = _settings["proxy_server"].strip()
        if "://" not in proxy:
            proxy = "socks5://" + proxy  # asume SOCKS5 si no se especifica esquema
        os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = (
            os.environ.get("QTWEBENGINE_CHROMIUM_FLAGS", "") + f" --proxy-server={proxy}"
        ).strip()
